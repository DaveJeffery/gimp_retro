...
#   BBC Micro (Mode 4) Image Filter - BBC Micro plugin for The GIMP.
...
def error_diffusion(dst_pxl, colours, width, height, pxl_size,
    ...
    for y in range(0, height):
        ...
        for x in range(0, width):
            ...
            # serpentine parsing
            sx = (width - x) - 1 if serpent and y % 2 else x
...
def ordered_dither(dst_pxl, colours, width, height, pxl_size,
    ...
    for y in range(0, height):
        ...
        for x in range(0, width):
            ...
            f = img_filter[y % fh][x % fw] * (strength / 100)
            r, g, b = r + f, g + f, b + f
...
def bbcmicro4(img, layer, strength, serpent, dither):
    ...
    # Define full palette
    full_palette = ((0x00, 0x00, 0x00), (0x00, 0x00, 0xFF),
    ...
    # Initialise progress bar
    gimp.progress_init("BBC Micro (Mode 4) Image Filter")
...
register("python-fu-bbcmicro4",
         N_("BBC Micro (Mode 4) Image Filter"),
         "",
         "Nitrofurano",
         "Nitrofurano",
         "2008",
         N_("_BBC Micro (Mode 4)"),
         "RGB*",
